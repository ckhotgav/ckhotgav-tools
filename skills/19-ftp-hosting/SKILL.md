...
| 免費域名 | `你的帳號.profreehost.com` | `你的帳號.infinityfreeapp.com` |
| FTP | ✅ 有控制台 | ✅ 有控制台 |
| MySQL | ✅ 支援 | ✅ 支援 |
| PHP | ✅ 支援 | ✅ 支援 |
| 空間 | 1GB | 5GB |
...
3. 去信箱收驗證信，點擊啟用連結
4. 登入控制台後，你的網址是 `https://你的帳號.profreehost.com`
...
5. 選擇域名：輸入你想要的帳號名稱 → `你的帳號.infinityfreeapp.com`
6. 密碼會自動產生，**記下來**（之後會用到）
7. 點「Create」完成
8. 之後登入控制台：`https://cp.infinityfree.com`（VistaPanel）
...
寫好後，把 `index.html` 用 FileZilla 拖曳到主機的根目錄：
- **ProFreeHost** → `public_html` 資料夾
- **InfinityFree** → `htdocs` 資料夾

### 確認網站上線

打開瀏覽器，輸入你的網址：
- **ProFreeHost**：`https://你的帳號.profreehost.com`
- **InfinityFree**：`https://你的帳號.infinityfreeapp.com`
